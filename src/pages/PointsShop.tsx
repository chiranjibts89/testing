...
import { ShoppingBag, Star, Crown, Zap, User, Award, Filter, Search } from 'lucide-react';
...
const PointsShop: React.FC = () => {
  const { currentStudent, addPoints } = useStudent();
  ...
  const shopItems: ShopItem[] = [
    ...
    },
    {
      ...
      category: 'avatars',
      rarity: 'legendary',
      imageUrl: 'https://images.pexels.com/photos/1239288/pexels-photo-1239288.jpeg?auto=compress&cs=tinysrgb&w=150&h=150&fit=crop'
    },
    ...
    },
    {
      ...
      category: 'powerups',
      rarity: 'legendary',
      imageUrl: 'https://images.pexels.com/photos/9875415/pexels-photo-9875415.jpeg?auto=compress&cs=tinysrgb&w=150'
    }
  ];
  ...
  const getRarityColor = (rarity: string) => {
    switch (rarity) {
      ...
      case 'legendary': return 'text-[#F8D991] bg-[#F8D991]/20';
  ...
  const getRarityBorder = (rarity: string) => {
    switch (rarity) {
      ...
      case 'legendary': return 'border-[#F8D991]/30';
      default: return 'border-gray-400/30';
    }
  };

  const handlePurchase = (item: ShopItem) => {
    if (currentStudent.ecoPoints >= item.price) {
      // Mock purchase logic
      console.log('Purchasing:', item.name);
      addPoints(-item.price); // Deduct points
      // In a real app, you'd update the user's inventory
    }
  };

  const canAfford = (price: number) => currentStudent.ecoPoints >= price;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredItems.map((item, index) => {
          ...
          return (
            ...
            >
              <GlassCard className={`p-4 border-2 ${getRarityBorder(item.rarity)} ${!affordable ? 'opacity-60' : ''}`}>
                <div className="relative">
                  ...
                  {/* Rarity Badge */}
                  <div className={`absolute top-2 right-2 px-2 py-1 rounded-full text-xs font-semibold ${getRarityColor(item.rarity)}`}>
                    {item.rarity}
                  </div>

                  {/* Category Icon */}
                  <div className="absolute top-2 left-2 w-8 h-8 bg-black/50 backdrop-blur-sm rounded-full flex items-center justify-center">
                    {item.category === 'avatars' && <User className="h-4 w-4 text-white" />}
                    {item.category === 'achievements' && <Award className="h-4 w-4 text-white" />}
                    {item.category === 'powerups' && <Zap className="h-4 w-4 text-white" />}
                  </div>
                </div>
